...
def free_bacon(score):
    ...
    if score < 10:
        ...
        score_str = str(score)
        ones = int(score_str[1])
        tens = int(score_str[0])
        if ones > tens:
            bacon_score = ones + 1
        elif tens > ones:
            bacon_score = tens + 1
    ...
    def say(score0, score1):
        ...
        if current_score > previous_score:
            ...
            if score_change > previous_high:
                if score_change == 1:
                    print(score_change, "point! That's the biggest gain yet for Player", who)
                else:
                    print(score_change, "points! That's the biggest gain yet for Player", who)
                return announce_highest(who, score_change, current_score)
            else:
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    for i in range(1,11):
        results.append(averaged_dice(i, dice))
    high = max(results)
    index = results.index(high)
    return index+1
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    #0 if would have beneficial swap, or if would give margin and not cause swap, else num rolls
    "*** REPLACE THIS LINE ***"
    return 0 if ((opponent_score % (score + free_bacon(opponent_score)) == 0) or (free_bacon(opponent_score) >= margin)) else num_rolls
    # END PROBLEM 11
