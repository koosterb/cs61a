        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.container is True:
                    if self.ant.ant is None:
                        insect.place = self
                        return self.ant.contain_ant(insect)
                    else:
                        assert self.ant is None, 'Too many ants in {0}'.format(self)
                elif insect.container is True:
                    if insect.ant is None:
                        temp = self.ant
                        self.ant = insect
                        insect.place = self
                        return self.ant.contain_ant(temp)
                    else:
                        assert self.ant is None, 'Too many ants in {0}'.format(self)
                else:
                    assert self.ant is None, 'Too many ants in {0}'.format(self)
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                if hasattr(self.ant, 'container') and self.ant.container:
                    ...
                    if self.ant.is_impostor is False:
                        return
                    else:
                        self.ant = None
                else:
                ...
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if isinstance(self.ant.ant, QueenAnt):
                        if self.ant.ant.is_impostor is False:
                            return
                        else:
                            self.ant.ant = None
                    else:
    ...
    def action(self, colony):
        ...
        if self.digesting == 0:
            if len(self.place.bees) != 0:
                choice = random_or_none(self.place.bees)
                return self.eat_bee(choice)
            else:
                return
        else:
            self.digesting -= 1
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        if self.is_impostor:
            return Ant.reduce_armor(self, self.armor)
        else:
            self.throw_at(self.nearest_bee(colony.hive))
            place = self.place
            ...
            while place is not None:
                if (place.ant is not None) and (place.ant not in self.doubled_list):
                    if place.ant.container is True:
                        if (place.ant.ant is not None) and (place.ant.ant not in self.doubled_list):
                            place.ant.damage *= 2
                            place.ant.ant.damage *= 2
                            self.doubled_list.append(place.ant)
                            self.doubled_list.append(place.ant.ant)
                            place = place.exit
                        else:
                            place.ant.damage *= 2
                            self.doubled_list.append(place.ant)
                        ...
                        self.doubled_list.append(place.ant)
                        place = place.exit
                elif (place.ant is not None) and (place.ant.container is True) and (place.ant in self.doubled_list):
                    if (place.ant.ant is not None) and (place.ant.ant not in self.doubled_list):
                        place.ant.ant.damage *= 2
                        self.doubled_list.append(place.ant.ant)
                        place = place.exit
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    #helper function, only does effect
