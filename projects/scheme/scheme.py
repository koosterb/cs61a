...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        return None
    to_eval = expressions
    while to_eval.second is not nil:
        scheme_eval(to_eval.first, env)
        to_eval = to_eval.second
    return scheme_eval(to_eval.first, env, True)
    # END PROBLEM 8
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Formals and vals lengths do not match!')
        cur_form = formals
        cur_val = vals
        while cur_form is not nil:
            child.define(cur_form.first, cur_val.first)
            cur_form = cur_form.second
            cur_val = cur_val.second
        # END PROBLEM 11
        return child
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        result = operands.map(lambda operand: scheme_eval(operand, env, False))
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            error = e
        if error:
            raise SchemeError('TypeError raised, check arguments again')
        # END PROBLEM 4
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        new_ops = self.apply(operands, env)
        result = complete_eval(new_ops)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    cur_exp = expressions
    while cur_exp.second is not nil:
        result = scheme_eval(cur_exp.first, env, False)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    cur_bind = bindings
    new_child = env.make_child_frame(nil, nil)
    while cur_bind is not nil:
        check_form(cur_bind.first, 2, 2)
        if not scheme_symbolp(cur_bind.first.first):
            raise SchemeError('non-symbol: {0}'.format(cur_bind.first.first))
        new_child.define(cur_bind.first.first, scheme_eval(cur_bind.first.second.first, new_child.parent))
        cur_bind = cur_bind.second
    return new_child
    # END PROBLEM 15
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    result = MuProcedure(formals, expressions.second)
    return result
    # END PROBLEM 16
