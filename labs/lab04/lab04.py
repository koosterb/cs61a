...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    cur_loc = make_city('current location', lat, lon)
    dist1 = distance(cur_loc, city1)
    dist2 = distance(cur_loc, city2)
    if dist1 < dist2:
...
def put_piece(board, max_rows, column, player):
    ...
    for i in range(1, max_rows+1):
        if board[max_rows-i][column] == '-':
            new_row = replace_elem(board[max_rows-i], column, player)
            new_board = replace_elem(board, max_rows-i, new_row)
            return max_rows-i, new_board
    return -1, board
